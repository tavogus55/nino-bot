...
class NTTConnector:
    def __init__(self, config_file):
        ...
        self.login_info = self.config['login_info']
        self.payment_info = self.config['payment_info']

    def logIn(self, driver):
        account_username = self.login_info['username']
        account_password = self.login_info['password']
        login_url = self.login_info['url']

        driver.get(login_url)

        input_username = driver.find_element(By.ID, "txtLoginId")
        input_password = driver.find_element(By.ID, "pwdLoginPsw")
